...
data_dir = '/home/pcl/Documents/Yaohua/LGC-Net/data/TUM/'
...
def display_test(dataset, mode):
    ...
    for i, seq in enumerate(dataset.sequences):
        ...
        my_net_us = pload(my_address_tum, seq, 'results.p')['hat_xs']
        DIG_net_us = pload(DIG_address_tum, seq, 'results.p')['hat_xs']
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(my_address_tum, seq, 'orientation' + '.png'))
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(my_address_tum, seq, 'orientation_error' + '.png'))
        ...
        if isinstance(axs, np.ndarray):
            ...
            axs.legend()
        fig.tight_layout()
        fig.savefig(os.path.join(my_address_tum, seq, 'gyro_correction' + '.png'))
        plt.show()
...
if __name__ == '__main__':
    ...
    dataset_params = {
        ...
        'N': 32 * 500,  # should be integer * 'max_train_freq'32 * 500
        'min_train_freq': 16,
        'max_train_freq': 32,
    }
    # define datasets
