...
class LearningBasedProcessing:
    ...
    def train(self, dataset_class, dataset_params, train_params):
        ...
        print('  + Number of params: %.3fM' % (total / 1e6))  # Mb

        total_train = sum([param.nelement() for param in self.net.parameters() if param.requires_grad])
        print('  + train Number of params: %.1f' % (total_train))  #
